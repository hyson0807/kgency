...
kgency의 메시징 시스템은 구직자와 기업 간의 원활한 소통을 위한 실시간 채팅 기능을 제공합니다. 지원서 기반 대화, 면접 조율, 채용 과정 전반의 커뮤니케이션을 지원합니다.
...
sequenceDiagram
    ...
    Note over U,C: 1. 지원서 제출 시 대화방 생성
    U->>A: 지원서 제출
    A->>DB: applications 테이블에 지원서 저장
    A->>DB: messages 테이블에 초기 메시지 생성
    
    Note over U,C: 2. 실시간 메시지 교환
    C->>A: 메시지 작성
    A->>DB: 메시지 저장
    DB->>RT: 실시간 이벤트 발생
    RT->>A: 구독자에게 알림
